...
for n in range(N):
    ...
    for m in range(1, len(x) - 1):
        ...
        f_next = f_u(u, m)
        u_half[0, m] = ((u[0, m]+u[0, m+1])-(k/h)*(f_next_p1[0]-f_next[0])+(k/2)*(s_next[0]+s_next_p1[0]))/2
        ...
        u_next[1, m] = u[0, m]-(k/h)*(f_u(u_half, m)[1]-f_u(u_half, m-1)[1])+(k/2)*(s(u_half, m, n)[1]+s(u_half, m-1, n)[1])
