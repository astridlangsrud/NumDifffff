...
def q(t): #Activating and deactivating vehicles from on-ramp
    if t < 1:
        return f_rmp
    elif 4<t<7:
        return f_rmp/4
...
U_matrix = np.zeros([N,len(x)]) #Declaration of matrix for storing density values
...
for n in range(N):
    U_matrix[n, :] = u[0, :-1] #Storing density values
...
#u[0] = u[0]*1000
#u[1] = u[1]*0.06
#rw.write_data(u, "L_W_ref_boundaries1")
#plt.plot(x,u[1][:-1])
